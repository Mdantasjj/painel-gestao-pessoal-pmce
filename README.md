...
Os BPMs e CRPMs também recebem uma referência territorial discreta baseada na aba `BASE` de `DISTRI VTR (1).xlsx`. Como a planilha não identifica formalmente a sede, cada unidade mostra somente o município vinculado de maior `area_km2`, sem rótulo adicional. A cidade acompanha todas as ocorrências visuais dessas unidades: gráfico de origens, composição do indicador, seletor, tabela e detalhamento individual.
