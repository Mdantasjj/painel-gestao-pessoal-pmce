...
- A coluna `Efetivo de licença saúde` foi alimentada pela relação funcional fornecida em 14/09/2026. Dos 875 registros sem matrículas duplicadas, 645 correspondem a LTS própria ou de dependente com vínculo direto a um dos 34 BPMs. O indicador exclui licenças gestante, paternidade e interesse particular, bem como registros de CRPMs e outras OPMs. Somente os quantitativos agregados são publicados; nomes e matrículas permanecem fora do repositório. A coluna ainda não integra os demais cálculos. A coluna `Déficit de movimentação` foi retirada da tabela; `Reestruturação necessidade` apresenta somente os resultados negativos obtidos após movimentações, exonerações e demissões.
