...
O card **BATALHÕES - Análise situacional de Efetivo** apresenta os 34 BPMs territoriais, classificados em 20 com ganho, 12 com perda e dois em equilíbrio nas movimentações. A subpágina mostra saldo conjunto, déficit localizado, Top 5 das maiores perdas e ganhos e uma tabela com posição, batalhão/cidades, efetivo total do batalhão, efetivo total do CRPM da região e situação média. Um painel de classificação permite ordenar interativamente por batalhão, efetivo do batalhão, efetivo do CRPM ou situação média e inverter a direção da ordem. A base consolidada cobre os 34 BPMs e os oito CRPMs, totalizando 9.956 policiais. A soma dos batalhões de cada região coincide com o total informado para o respectivo CRPM. A situação média continua baseada nas movimentações do período e não deve ser confundida com o efetivo existente. O estudo específico da reestruturação permanece baseado no recorte estrutural das nove unidades do interior e do litoral.
