...
O card **BATALHÕES - Análise situacional de Efetivo** apresenta os 34 BPMs territoriais, classificados em 20 com ganho, 12 com perda e dois em equilíbrio nas movimentações. A subpágina mostra o ranking das movimentações e uma tabela ampliada com posição, batalhão/cidades, efetivo total do batalhão, efetivo total do CRPM da região, situação média, exonerações, demissões, perda líquida nas movimentações, aposentadorias e déficit apurado. O cálculo territorial reúne 111 perdas líquidas nas movimentações, 62 exonerações e 170 demissões, totalizando 343 policiais. As 207 aposentadorias permanecem informadas, mas não foram distribuídas entre os BPMs porque a fonte não identifica a unidade de origem. Um painel de classificação permite ordenar interativamente pelos principais campos numéricos e inverter a direção da ordem. A base consolidada cobre os 34 BPMs e os oito CRPMs, totalizando 9.956 policiais. A soma dos batalhões de cada região coincide com o total informado para o respectivo CRPM.
